...
Knowing the above $$Q-$$values, we just need to choose the actions with the highes $$Q-$$values at each state to reach to the destination as soon as possible. The code for tabular Q-learning can be found [here](https://github.com/abhishm/dqn/tree/master/tabular_q_learning).
